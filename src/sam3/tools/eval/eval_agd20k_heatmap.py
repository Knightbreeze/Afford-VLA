...
def save_heatmap_overlay(image, heatmap_logits, save_path, apply_sigmoid=True, smooth_sigma=6, threshold=0.2):
    ...
    alpha_mask_raw = np.where(
        heatmap_probs_smooth >= threshold * 0.5,
...
def batch_process(processor, affordance_label, object_name):
    ...
    for idx, img_file in enumerate(image_files, 1):
        try:
            ...
            save_heatmap_overlay(
                image=image, heatmap_logits=full_heatmap, save_path=heatmap_path, apply_sigmoid=False, threshold=0.5
            )

            # 保存单张热力图叠加结果
            heatmap_result_path = (
                Path(HEATMAP_RESULT_DIR) / affordance_label / object_name / img_file.name.replace(".jpg", ".png")
            )
            save_single_heatmap_overlay(
                image=image,
                heatmap_logits=full_heatmap,
