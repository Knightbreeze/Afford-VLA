...
def compare_single_vs_batch():
    ...
    print(f"  Heatmap logits range: [{output_single['full_heatmap_logits'].min():.6f}, {output_single['full_heatmap_logits'].max():.6f}]")
    ...
    with torch.no_grad():
        ...
        output_batch = processor.set_text_prompt_batch(
    ...
    print(f"  Heatmap logits range: [{output_batch['full_heatmap_logits'][0].min():.6f}, {output_batch['full_heatmap_logits'][0].max():.6f}]")
