        ...
        for layer_idx, layer in enumerate(self.fusion_encoder.layers):
            ...
            def make_hook(idx):
                def hook_fn(module, input, output):
                    ...
                    self.layer_outputs.append({"layer_idx": idx, "features": captured})
...
def feature_to_heatmap(
    ...
    if use_threshold:
        ...
        # 创建平滑的alpha mask
        alpha_mask_raw = np.where(
            heatmap_np >= threshold * 0.5, np.clip((heatmap_np - threshold * 0.5) / (threshold * 0.5 + 1e-8), 0, 1), 0
        )
        if smooth_sigma > 0:
...
def visualize_all_layers(image_np, layer_outputs, prompt, save_dir, invert=True, smooth_sigma=0, aggregation="l2"):
    ...
    for subplot_idx, layer_idx in enumerate(layer_indices, start=1):
        ...
        heatmap, alpha_mask = feature_to_heatmap(
            ...
            high_heat_min=HIGH_HEAT_MIN,
            high_heat_max=HIGH_HEAT_MAX,
            gamma=CONTRAST_GAMMA,
        )

        # 绘制
        im = axes[subplot_idx].imshow(heatmap, cmap="jet", vmin=0, vmax=1)
        axes[subplot_idx].set_title(f'Layer {layer_data["layer_idx"]} Features', fontsize=12)
    ...
    if num_layers > 0:
        ...
        title_parts = ["Final Layer Overlay", f'"{prompt}"']
...
def visualize_final_layer_detailed(
    image_np, layer_outputs, prompt, save_dir, invert=True, smooth_sigma=0, aggregation="l2"
):
    ...
    title_parts = ["Fusion Encoder Features", f'(Layer {last_layer["layer_idx"]}, L2 Norm{threshold_info})']
