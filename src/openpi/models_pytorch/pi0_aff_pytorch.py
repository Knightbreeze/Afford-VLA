...
class PI0Pytorch(nn.Module):
    def __init__(self, config, extra_config=None):
        ...
        # extra_config for AffordanceSegment alignment
        if extra_config is not None:
            self.aff_input_image_key = extra_config.aff_input_image_key
            self.vla_layers_align = extra_config.vla_layers_align
            self.aff_layers_align = extra_config.aff_layers_align
            self.use_cross_attn_align = False

            if self.use_cross_attn_align:
                # cross-attn align
                self.cross_attn_align_loss = CrossAttentionAlignProjector(
                    num_heads=getattr(extra_config, "align_num_heads", 8),
                    loss_type=getattr(extra_config, "align_loss_type", "mse"),
                    dropout=getattr(extra_config, "align_dropout", 0.1),
                )
            else:
                # MLP
    ...
    def forward(self, observation, actions, aff_prompts, aff_processor, align_proj, noise=None, time=None) -> Tensor:
        ...
        third_view_image_batch = original_images[self.aff_input_image_key[0]]
        ...
        aff_img_pos_embed = aff_output["pos_embed"]
