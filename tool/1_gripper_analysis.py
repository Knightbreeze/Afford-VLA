...
class GripperConfig:
    ...
    def __post_init__(self):
        if self.one_means_open:
            assert self.open_threshold > self.close_threshold, "当1表示打开时,open_threshold应该大于close_threshold"
        else:
            assert self.close_threshold > self.open_threshold, "当1表示闭合时,close_threshold应该大于open_threshold"
...
def detect_gripper_changes(gripper_values: np.ndarray, config: GripperConfig) -> dict:
    """检测夹爪状态变化
...
def plot_gripper_states(
    ...
    save_path: str = "episode_gripper_analysis.png",
) -> None:
    """可视化夹爪状态"""
    ...
    axes[0].axhline(
        y=config.open_threshold, color="g", linestyle="--", alpha=0.5, label=f"Open Threshold ({config.open_threshold})"
    )
    axes[0].axhline(
        y=config.close_threshold,
        color="r",
        linestyle="--",
        alpha=0.5,
...
def save_results_to_csv(
    df: pd.DataFrame, left_info: dict, right_info: dict, save_path: str = "episode_gripper_events.csv"
) -> None:
    """将夹爪事件保存到CSV文件"""
    events = []
    ...
    for op in left_info["open_points"]:
        op = int(op)
        events.append({"frame_index": op, "gripper": "left", "event": "open", "value": float(left_info["values"][op])})

    for cp in left_info["close_points"]:
        cp = int(cp)
        events.append({"frame_index": cp, "gripper": "left", "event": "close", "value": float(left_info["values"][cp])})

    # 右手事件
    for op in right_info["open_points"]:
        op = int(op)
        events.append(
            {"frame_index": op, "gripper": "right", "event": "open", "value": float(right_info["values"][op])}
        )

    for cp in right_info["close_points"]:
        cp = int(cp)
        events.append(
            {"frame_index": cp, "gripper": "right", "event": "close", "value": float(right_info["values"][cp])}
        )
...
def process_single_episode(file_path: str, config: GripperConfig, output_dir: Path | None = None) -> dict:
    ...
    return {"episode_index": episode_index, "gripper_change": gripper_changes}
...
def merge_gripper_events(left_info: dict, right_info: dict, frame_threshold: int = 3) -> list[dict]:
    ...
    # 添加左手事件
    for op in left_info["open_points"]:
        all_events.append({"gripper_step": int(op), "gripper": "left", "event": "open"})

    for cp in left_info["close_points"]:
        all_events.append({"gripper_step": int(cp), "gripper": "left", "event": "close"})

    # 添加右手事件
    for op in right_info["open_points"]:
        all_events.append({"gripper_step": int(op), "gripper": "right", "event": "open"})

    for cp in right_info["close_points"]:
        all_events.append({"gripper_step": int(cp), "gripper": "right", "event": "close"})
    ...
    for i, event in enumerate(all_events):
        ...
        for j in range(i + 1, len(all_events)):
            ...
            # 检查是否是同类事件且是另一只手
            if other_event["event"] == current_event_type and other_event["gripper"] != current_gripper:
                matched_idx = j
                break
        ...
        if matched_idx is not None:
            ...
            # 没有匹配的事件，单独保存
            merged_events.append(
                {"gripper_step": current_step, "gripper": current_gripper, "event": current_event_type}
            )
            used_indices.add(i)
