...
def process_prediction(prediction, spatial_rules=None):
    ...
    if spatial_rules:
        ...
        if len(check_indices) != len(check_strings) or len(check_indices) != len(modify_strings):
            print(f"  ⚠ Warning: Rule lists have different lengths, skipping rules")
...
def merge_aff_prompts(dataset_name, task_name, spatial_rules=None):
    ...
    first_aff_data = {}
    with open(first_aff_path, 'r', encoding='utf-8') as f:
        for line in f:
    ...
    for episode_index in sorted(first_aff_data.keys()):
        ...
        if episode_index not in vlm_data:
            print(f"  ✗ Warning: No VLM prediction found, skipping...")
            continue
        ...
        if prediction_list is None:
            print(f"  ✗ Failed to process prediction, skipping...")
            continue
        
        # 验证长度匹配
        if len(prediction_list) != first_data['prompts_num']:
            print(f"  ✗ Warning: Prediction length ({len(prediction_list)}) != prompts_num ({first_data['prompts_num']})")
            continue
