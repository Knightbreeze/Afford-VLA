...
def read_gripper_events(gripper_events_path: str) -> List[Dict]:
    ...
    with open(gripper_events_path, 'r', encoding='utf-8') as f:
        for line in f:
...
def generate_time_segments(gripper_changes: List[Dict], 
                          total_frames: int,
                          num_time_segments: int = 3) -> List[Dict]:
    """根据夹爪变化生成时间段
...
def generate_aff_prompts(dataset_name: str,
                        task_name: str,
                        num_time_segments: int = 3,
    ...
    for episode_data in episodes_gripper_data:
        ...
        aff_prompt_data = {
            ...
            "prompts_num": len(segments),
            "aff_prompts": segments
        }
